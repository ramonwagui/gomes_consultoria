...
exports.cancelarSincronizacaoTransferenciasDiscricionarias = exports.complementarConveniosAusentesTransferenciasDiscricionarias = exports.obterStatusSincronizacaoTransferenciasDiscricionarias = exports.sincronizarTransferenciasDiscricionarias = exports.listarSugestoesProponentePorCnpj = exports.listarDesembolsosPorProponenteTransferenciasDiscricionarias = exports.listarDesembolsosTransferenciasDiscricionarias = exports.listarTransferenciasDiscricionarias = exports.listarFiltrosTransferenciasDiscricionarias = exports.ensureTransferenciasDiscricionariasStorage = exports.detectCsvEncoding = exports.processCsvBytesByLine = exports.downloadCsvTextFromZipWithRetry = exports.downloadCsvBytesFromZipWithRetry = exports.splitCsvLine = void 0;
const client_1 = require("@prisma/client");
const fflate_1 = require("fflate");
const env_1 = require("../../config/env");
const prisma_1 = require("../../lib/prisma");
const convenetes_service_1 = require("../convenetes/convenetes.service");
const transferencias_discricionarias_changes_notify_service_1 = require("./transferencias-discricionarias-changes-notify.service");
const obras_service_1 = require("./obras.service");
const isDemoUser = (user) => user?.role === client_1.UserRole.DEMONSTRACAO;
...
const VIGENCIA_FIM_NORMALIZED_SQL = "NULLIF(regexp_replace(TRIM(dia_fim_vigencia), '[ T].*$', ''), '')";
const VIGENCIA_FIM_DATE_SQL = "CASE " +
...
const toNullableText = (value) => {
    ...
    const trimAndCompact = (input) => input.trim().replace(/\s+/g, " ");
    const maybeFixMojibake = (input) => {
        if (!/[ÃÂ]/.test(input)) {
            return input;
        }
        try {
...
const extractYearFromDateBr = (value) => {
    if (!value) {
        return null;
    }
...
const normalizeCsvHeaderLine = (line) => line.replace(/^(?:\uFEFF|ï»¿)/, "");
const extractYearFromProposta = (value) => {
    if (!value) {
        return null;
    }
...
const resolveTipoEnte = (fileName) => {
    const lower = fileName.toLowerCase();
    return lower.includes("municip") ? "municipio" : "estado";
};
const normalizeBaseUrl = () => {
    const base = env_1.env.transferenciasDiscricionariasSourceBaseUrl.trim();
    return base.endsWith("/") ? base.slice(0, -1) : base;
};
const getSourceFiles = () => {
    const raw = env_1.env.transferenciasDiscricionariasSourceFiles;
    const files = raw
        .split(",")
        .map((item) => item.trim())
        .filter((item) => item !== "")
...
const downloadTextWithRetry = async (url) => {
    const timeoutMs = Math.max(5000, env_1.env.transferenciasDiscricionariasRequestTimeoutMs);
    const retries = Math.max(1, env_1.env.transferenciasDiscricionariasDownloadRetries);
    let lastError = null;
...
const downloadCsvBytesFromZipWithRetry = async (fileName) => {
    const timeoutMs = Math.max(10 * 60 * 1000, env_1.env.transferenciasDiscricionariasRequestTimeoutMs);
    const retries = Math.max(1, env_1.env.transferenciasDiscricionariasDownloadRetries);
    ...
    for (let attempt = 1; attempt <= retries; attempt += 1) {
        try {
            ...
            const unzipped = (0, fflate_1.unzipSync)(rawBuffer);
...
exports.downloadCsvBytesFromZipWithRetry = downloadCsvBytesFromZipWithRetry;
const downloadCsvTextFromZipWithRetry = async (fileName, encoding = "utf-8") => {
    const csvBytes = await (0, exports.downloadCsvBytesFromZipWithRetry)(fileName);
    return new TextDecoder(encoding).decode(csvBytes);
};
exports.downloadCsvTextFromZipWithRetry = downloadCsvTextFromZipWithRetry;
const processCsvBytesByLine = async (csvBytes, encoding, onLine) => {
...
exports.processCsvBytesByLine = processCsvBytesByLine;
const detectCsvEncoding = (csvBytes) => {
...
const ensureTables = async () => {
    if (!tablesReadyPromise) {
      ...
      `);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_uf ON ${TABLE_MAIN} (uf);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_ano ON ${TABLE_MAIN} (ano_referencia);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_cnpj ON ${TABLE_MAIN} (cnpj);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_nr_convenio ON ${TABLE_MAIN} (nr_convenio);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_nr_proposta ON ${TABLE_MAIN} (nr_proposta);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_sit_prop ON ${TABLE_MAIN} (situacao_proposta);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_sit_conv ON ${TABLE_MAIN} (situacao_convenio);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_tipo_ente ON ${TABLE_MAIN} (tipo_ente);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_desembolso_conv ON ${TABLE_DESEMBOLSO} (nr_convenio);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_desembolso_data ON ${TABLE_DESEMBOLSO} (data_desembolso);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_desembolso_ano_mes ON ${TABLE_DESEMBOLSO} (ano_desembolso, mes_desembolso);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_prop_stage_id ON ${TABLE_PROPOSTA_STAGE} (id_proposta);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_prop_stage_nr_norm ON ${TABLE_PROPOSTA_STAGE} (nr_proposta_norm);`);
            const ensureOptionalColumn = async (tableName, columnName, sqlType) => {
                try {
                    await prisma_1.prisma.$executeRawUnsafe(`ALTER TABLE ${tableName} ADD COLUMN ${columnName} ${sqlType}`);
                }
                catch {
                    // coluna ja existente
                }
      ...
      `);
        ...
        WHERE nr_convenio IS NOT NULL
          ...
          AND (nr_convenio_norm IS NULL OR TRIM(nr_convenio_norm) = '')
      `);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_nr_convenio_norm ON ${TABLE_MAIN} (nr_convenio_norm);`);
            await prisma_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_td_desembolso_conv_norm ON ${TABLE_DESEMBOLSO} (nr_convenio_norm);`);
            await prisma_1.prisma.$executeRaw(client_1.Prisma.sql `
          INSERT INTO ${client_1.Prisma.raw(TABLE_SYNC)} (
            id,
            data_carga_fonte,
            atualizado_em,
            status,
...
const ensureTransferenciasDiscricionariasStorage = async () => {
    await ensureTables();
};
exports.ensureTransferenciasDiscricionariasStorage = ensureTransferenciasDiscricionariasStorage;
const readSyncState = async () => {
    await ensureTables();
    const rows = await prisma_1.prisma.$queryRaw(client_1.Prisma.sql `SELECT data_carga_fonte, atualizado_em, status, detalhe, total_registros, fase_atual, progresso_percentual, heartbeat_em, mode, cancel_requested, run_id, published_run_id FROM ${client_1.Prisma.raw(TABLE_SYNC)} WHERE id = 1`);
    const row = rows[0];
    if (!row) {
        return {
...
const isSyncRunningStale = (state, nowMs = Date.now()) => {
...
const resolveSyncOptions = (input) => {
...
const updateSyncRun = async (runId, patch) => {
...
const createSyncRun = async (runId, dataCargaFonte, mode) => {
    await prisma_1.prisma.$executeRaw `
...
const getStageQualityMetrics = async () => {
...
const pct = (value, total) => (total <= 0 ? 0 : value / total);
const validateStageBeforePublish = async (warnings, sourceFiles) => {
    ...
    const failedCriticalFiles = criticalFiles.filter((file) => lowerWarnings.some((warning) => warning.includes(file)));
    ...
    if (metrics.total_anterior > 0 && metrics.total < Math.floor(metrics.total_anterior * 0.8)) {
        reasons.push(`Total da carga nova (${metrics.total}) abaixo de 80% da carga anterior (${metrics.total_anterior}).`);
    }
    ...
    if (metrics.banco_anterior > 0 && metrics.banco_preenchido < Math.floor(metrics.banco_anterior * 0.9)) {
        reasons.push(`Banco preenchido caiu abaixo de 90% da carga anterior (${metrics.banco_preenchido}/${metrics.banco_anterior}).`);
    }
    if (metrics.agencia_anterior > 0 && metrics.agencia_preenchida < Math.floor(metrics.agencia_anterior * 0.9)) {
        reasons.push(`Agencia preenchida caiu abaixo de 90% da carga anterior (${metrics.agencia_preenchida}/${metrics.agencia_anterior}).`);
    }
    return {
        ok: reasons.length === 0,
        reasons,
...
const snapshotCurrentPublishedData = async (runId, createdAt) => {
    const currentCountRows = await prisma_1.prisma.$queryRaw `
...
const toInsertSqlRow = (row) => client_1.Prisma.sql `(
  ${row.chave_unica},
  ${row.fonte_arquivo},
...
const insertStageBatch = async (rows) => {
    if (rows.length === 0) {
        return;
    }
    await prisma_1.prisma.$executeRaw(client_1.Prisma.sql `
      INSERT INTO ${client_1.Prisma.raw(TABLE_STAGE)} (
        chave_unica,
        fonte_arquivo,
        data_carga_fonte,
        tipo_ente,
      ...
      VALUES ${client_1.Prisma.join(rows.map((row) => toInsertSqlRow(row)))}
    `);
};
const toInsertDesembolsoSqlRow = (row) => client_1.Prisma.sql `(
  ${row.chave_unica},
  ${row.fonte_arquivo},
  ${row.data_carga_fonte},
  ${row.id_desembolso},
...
const insertDesembolsoStageBatch = async (rows) => {
    if (rows.length === 0) {
        return;
    }
    await prisma_1.prisma.$executeRaw(client_1.Prisma.sql `
      INSERT INTO ${client_1.Prisma.raw(TABLE_DESEMBOLSO_STAGE)} (
        chave_unica,
        fonte_arquivo,
        data_carga_fonte,
        id_desembolso,
      ...
      VALUES ${client_1.Prisma.join(rows.map((row) => toInsertDesembolsoSqlRow(row)))}
    `);
};
const toInsertContrapartidaSqlRow = (row) => client_1.Prisma.sql `(${row.nr_convenio}, ${row.valor_contrapartida_financeira}, ${row.dia_limite_prest_contas})`;
const insertContrapartidaStageBatch = async (rows) => {
    if (rows.length === 0) {
        return;
    }
...
const enriquecerContrapartidaFinanceiraNaStage = async (conveniosValidos) => {
    const convenioFile = "siconv_convenio.csv.zip";
    const csvText = await (0, exports.downloadCsvTextFromZipWithRetry)(convenioFile, "latin1");
    await prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_CONTRAPARTIDA_STAGE}`);
    const lines = csvText.split(/\r?\n/);
    const headerLine = normalizeCsvHeaderLine(lines[0] ?? "");
    const headerCells = (0, exports.splitCsvLine)(headerLine);
    const headerIndexMap = new Map(headerCells.map((column, index) => [column.trim(), index]));
    const getCell = (column, cells) => {
    ...
    for (let lineIndex = 1; lineIndex < lines.length; lineIndex += 1) {
        ...
        const cells = (0, exports.splitCsvLine)(line);
...
const insertIngressoContrapartidaStageBatch = async (rows) => {
    if (rows.length === 0) {
        return;
    }
    const aggregatedRows = Array.from(rows.reduce((acc, row) => {
        acc.set(row.nr_convenio, (acc.get(row.nr_convenio) ?? 0) + row.valor_contrapartida_depositada);
        return acc;
    }, new Map())).map(([nr_convenio, valor_contrapartida_depositada]) => ({
        nr_convenio,
        valor_contrapartida_depositada
    }));
...
const enriquecerIngressoContrapartidaNaStage = async (conveniosValidos) => {
    const csvText = await (0, exports.downloadCsvTextFromZipWithRetry)(INGRESSO_CONTRAPARTIDA_FILE, "latin1");
    await prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_INGRESSO_CONTRAPARTIDA_STAGE}`);
    const lines = csvText.split(/\r?\n/);
    const headerLine = normalizeCsvHeaderLine(lines[0] ?? "");
    const headerCells = (0, exports.splitCsvLine)(headerLine);
    const headerIndexMap = new Map(headerCells.map((column, index) => [column.trim(), index]));
...
const toInsertPropostaSqlRow = (row) => client_1.Prisma.sql `(
  ${row.chave_unica},
  ${row.id_proposta},
  ${row.nr_proposta},
...
const insertPropostaStageBatch = async (rows) => {
    if (rows.length === 0) {
        return;
    }
    const values = rows.map(row => {
...
const resolveTipoEnteByNatureza = (naturezaJuridica) => {
...
const complementarStageComConveniosAusentes = async (dataCargaFonte) => {
    const propostas = await prisma_1.prisma.$queryRaw(client_1.Prisma.sql `
    SELECT
      id_proposta,
      nr_proposta,
      uf,
      ...
      concedente_orgao_sup,
      concedente_orgao
    FROM ${client_1.Prisma.raw(TABLE_PROPOSTA_STAGE)}
  ...
  `);
    ...
    const propostaById = new Map();
    for (const proposta of propostas) {
        propostaById.set(proposta.id_proposta, proposta);
    }
    const existingPairs = await prisma_1.prisma.$queryRaw(client_1.Prisma.sql `
    SELECT DISTINCT (COALESCE(id_proposta::text, '') || '|' || COALESCE(nr_convenio_norm, '')) AS pair_key
    FROM ${client_1.Prisma.raw(TABLE_STAGE)}
  `);
    const existingPairKeys = new Set(existingPairs.map((item) => item.pair_key));
    const csvBytes = await (0, exports.downloadCsvBytesFromZipWithRetry)("siconv_convenio.csv.zip");
    const encoding = (0, exports.detectCsvEncoding)(csvBytes);
    ...
    await (0, exports.processCsvBytesByLine)(csvBytes, encoding, async (line, lineIndex) => {
        if (lineIndex === 0) {
        ...
        const anoReferencia = extractYearFromDateBr(diaAssinConv) ??
        ...
        batch.push({
            ...
            valor_contrapartida_financeira: toNullableFloat(getCell("VL_CONTRAPARTIDA_CONV", cells)) ?? proposta.valor_contrapartida_financeira,
...
const enriquecerDadosPropostaNaStage = async (propostasValidas, cnpjsValidos, dataCargaFonte) => {
    const csvBytes = await (0, exports.downloadCsvBytesFromZipWithRetry)(PROPOSTA_FILE);
    const encoding = (0, exports.detectCsvEncoding)(csvBytes);
    await prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_PROPOSTA_STAGE}`);
    let totalInserted = 0;
    let batch = [];
    let headerIndexMap = null;
    const getCell = (column, cells) => {
    ...
    const nrPropostaStageNormExpr = `REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(${TABLE_STAGE}.nr_proposta, ''), '.', ''), '/', ''), '-', ''), ' ', ''), ',', '')`;
    await prisma_1.prisma.$executeRawUnsafe(`
    UPDATE ${TABLE_STAGE}
    SET
...
const processCsvIntoStage = async (csvText, fileName, dataCargaFonte, cnpjsValidos, conveniosValidos, propostasValidas) => {
...
const processCsvDesembolsoIntoStage = async (csvText, fileName, dataCargaFonte, conveniosValidos) => {
    const lines = csvText.split(/\r?\n/);
    const headerLine = normalizeCsvHeaderLine(lines[0] ?? "");
    const headerCells = (0, exports.splitCsvLine)(headerLine);
    const headerIndexMap = new Map(headerCells.map((column, index) => [column.trim(), index]));
    ...
    for (let lineIndex = 1; lineIndex < lines.length; lineIndex += 1) {
        ...
        const baseKey = idDesembolso !== null
            ? `id:${idDesembolso}`
...
const mapRawDesembolsoListItem = (item) => ({
    id: toInt(item.id),
...
const mapRawDesembolsoProponenteListItem = (item) => ({
    id: toInt(item.id),
    id_desembolso: item.id_desembolso === null ? null : toInt(item.id_desembolso),
...
const loadDistinctTextValues = async (columnName, cnpjsPermitidos) => {
    await ensureTables();
    let whereClause = client_1.Prisma.sql `WHERE ${client_1.Prisma.raw(columnName)} IS NOT NULL AND TRIM(${client_1.Prisma.raw(columnName)}) <> ''`;
    if (cnpjsPermitidos && cnpjsPermitidos.size > 0) {
        const cnpjList = [...cnpjsPermitidos].map((c) => client_1.Prisma.sql `${c}`);
        whereClause = client_1.Prisma.sql `${whereClause} AND cnpj IN (${client_1.Prisma.join(cnpjList, ",")})`;
    }
    const rows = await prisma_1.prisma.$queryRaw(client_1.Prisma.sql `
    SELECT DISTINCT ${client_1.Prisma.raw(columnName)} AS value
    FROM ${client_1.Prisma.raw(TABLE_MAIN)}
    ${whereClause}
    ORDER BY ${client_1.Prisma.raw(columnName)} ASC
  `);
    return rows.map((row) => row.value?.trim()).filter((value) => Boolean(value));
};
const loadDistinctConcedentes = async (cnpjsPermitidos) => {
    await ensureTables();
    let whereClause = client_1.Prisma.sql `WHERE COALESCE(NULLIF(TRIM(concedente_orgao_sup), ''), NULLIF(TRIM(concedente_orgao), '')) IS NOT NULL`;
    if (cnpjsPermitidos && cnpjsPermitidos.size > 0) {
        const cnpjList = [...cnpjsPermitidos].map((c) => client_1.Prisma.sql `${c}`);
        whereClause = client_1.Prisma.sql `${whereClause} AND cnpj IN (${client_1.Prisma.join(cnpjList, ",")})`;
    }
...
const listarFiltrosTransferenciasDiscricionarias = async (user) => {
    // Se for usuário demo, filtrar apenas pelos CNPJs do ambiente demo
    let cnpjsDoAmbiente;
    if (isDemoUser(user)) {
        const convenetes = await prisma_1.prisma.convenete.findMany({
...
exports.listarFiltrosTransferenciasDiscricionarias = listarFiltrosTransferenciasDiscricionarias;
const listarTransferenciasDiscricionarias = async (query, user) => {
    await ensureTables();
    // Se for usuário demo, filtrar apenas pelos CNPJs do ambiente demo
    let cnpjsDoAmbiente;
    if (isDemoUser(user)) {
    ...
    const conditions = [];
    const vigenciaFimDateExpr = client_1.Prisma.raw(VIGENCIA_FIM_DATE_SQL);
    if (query.uf) {
        conditions.push(client_1.Prisma.sql `uf = ${query.uf}`);
    }
    if (query.ano !== undefined) {
        conditions.push(client_1.Prisma.sql `ano_referencia = ${query.ano}`);
    }
    if (query.tipo_ente) {
        conditions.push(client_1.Prisma.sql `tipo_ente = ${query.tipo_ente}`);
    }
    if (query.cnpj) {
        const cnpjDigits = query.cnpj.replace(/\D/g, "");
        conditions.push(client_1.Prisma.sql `REPLACE(REPLACE(REPLACE(REPLACE(cnpj, '.', ''), '/', ''), '-', ''), ' ', '') = ${cnpjDigits}`);
    }
    // Filtrar por CNPJs do ambiente demo
    if (cnpjsDoAmbiente && cnpjsDoAmbiente.size > 0) {
        const cnpjList = [...cnpjsDoAmbiente].map((c) => client_1.Prisma.sql `${c}`);
        conditions.push(client_1.Prisma.sql `cnpj IN (${client_1.Prisma.join(cnpjList, ",")})`);
    }
    const likeFilter = (columnName, value) => {
        if (value) {
            const normalized = `%${escapeLikeValue(value.toLowerCase().trim().replace(/\s+/g, ' '))}%`;
            conditions.push(client_1.Prisma.sql `unaccent(LOWER(regexp_replace(${client_1.Prisma.raw(columnName)}, '\\s+', ' ', 'g'))) LIKE unaccent(${normalized}) ESCAPE '\\'`);
        }
    };
    const likeOrDigitsFilter = (columnName, value) => {
        if (!value) {
            return;
        }
        ...
        const digits = value.replace(/\D/g, "");
        if (digits.length === 0) {
            conditions.push(client_1.Prisma.sql `LOWER(${client_1.Prisma.raw(columnName)}) LIKE ${normalized} ESCAPE '\\'`);
            return;
        }
        const digitsLike = `%${digits}%`;
        const columnDigits = client_1.Prisma.sql `REPLACE(REPLACE(REPLACE(REPLACE(${client_1.Prisma.raw(columnName)}, '.', ''), '/', ''), '-', ''), ' ', '')`;
        conditions.push(client_1.Prisma.sql `(LOWER(${client_1.Prisma.raw(columnName)}) LIKE ${normalized} ESCAPE '\\' OR ${columnDigits} LIKE ${digitsLike})`);
    };
    ...
    if (query.vigencia_a_vencer_dias !== undefined) {
        conditions.push(client_1.Prisma.sql `${vigenciaFimDateExpr} IS NOT NULL`);
        conditions.push(client_1.Prisma.sql `${vigenciaFimDateExpr} >= CURRENT_DATE`);
        conditions.push(client_1.Prisma.sql `${vigenciaFimDateExpr} <= CURRENT_DATE + CAST(${query.vigencia_a_vencer_dias} AS INTEGER)`);
    }
    const whereClause = conditions.length > 0 ? client_1.Prisma.sql `WHERE ${client_1.Prisma.join(conditions, " AND ")}` : client_1.Prisma.empty;
    const orderByClause = query.vigencia_a_vencer_dias !== undefined
        ? client_1.Prisma.sql `ORDER BY COALESCE(CAST((${vigenciaFimDateExpr} - CURRENT_DATE) AS INTEGER), 999999) ASC, id DESC`
        : client_1.Prisma.sql `ORDER BY COALESCE(ano_referencia, 0) DESC, id DESC`;
    const page = query.page;
    const pageSize = query.page_size;
    const offset = (page - 1) * pageSize;
    const countRows = await prisma_1.prisma.$queryRaw(client_1.Prisma.sql `SELECT COUNT(*) AS total FROM ${client_1.Prisma.raw(TABLE_MAIN)} ${whereClause}`);
...
exports.listarTransferenciasDiscricionarias = listarTransferenciasDiscricionarias;
const listarDesembolsosTransferenciasDiscricionarias = async (query) => {
    await ensureTables();
    const conditions = [];
    const nrConvenioNorm = normalizeConvenioCode(query.nr_convenio);
    if (nrConvenioNorm) {
        conditions.push(client_1.Prisma.sql `nr_convenio_norm LIKE ${`%${nrConvenioNorm}%`}`);
    }
    else {
        const normalized = `%${escapeLikeValue(query.nr_convenio.toLowerCase())}%`;
        conditions.push(client_1.Prisma.sql `LOWER(nr_convenio) LIKE ${normalized} ESCAPE '\\'`);
    }
    if (query.ano !== undefined) {
        conditions.push(client_1.Prisma.sql `ano_desembolso = ${query.ano}`);
    }
    if (query.mes !== undefined) {
        conditions.push(client_1.Prisma.sql `mes_desembolso = ${query.mes}`);
    }
    const whereClause = conditions.length > 0 ? client_1.Prisma.sql `WHERE ${client_1.Prisma.join(conditions, " AND ")}` : client_1.Prisma.empty;
    const page = query.page;
    const pageSize = query.page_size;
    ...
    const [countRows, rows, resumoRows, syncState] = await Promise.all([
        prisma_1.prisma.$queryRaw(client_1.Prisma.sql `SELECT COUNT(*) AS total FROM ${client_1.Prisma.raw(TABLE_DESEMBOLSO)} ${whereClause}`),
      ...
      SELECT
        ...
        vl_desembolsado,
        fonte_arquivo
      FROM ${client_1.Prisma.raw(TABLE_DESEMBOLSO)}
      ${whereClause}
...
exports.listarDesembolsosTransferenciasDiscricionarias = listarDesembolsosTransferenciasDiscricionarias;
const listarDesembolsosPorProponenteTransferenciasDiscricionarias = async (query) => {
    await ensureTables();
    const conditions = [];
    if (query.cnpj) {
        const cnpjDigits = query.cnpj.replace(/\D/g, "").trim();
        if (cnpjDigits.length > 0) {
            conditions.push(client_1.Prisma.sql `p.cnpj LIKE ${`%${cnpjDigits}%`}`);
        }
    }
    if (query.nome_proponente) {
        const nomeLike = `%${escapeLikeValue(query.nome_proponente.toLowerCase())}%`;
        conditions.push(client_1.Prisma.sql `LOWER(p.nome_proponente) LIKE ${nomeLike} ESCAPE '\\'`);
    }
    if (query.ano !== undefined) {
        conditions.push(client_1.Prisma.sql `d.ano_desembolso = ${query.ano}`);
    }
    if (query.mes !== undefined) {
        conditions.push(client_1.Prisma.sql `d.mes_desembolso = ${query.mes}`);
    }
    const whereClause = conditions.length > 0 ? client_1.Prisma.sql `WHERE ${client_1.Prisma.join(conditions, " AND ")}` : client_1.Prisma.empty;
    const fromClause = client_1.Prisma.sql `
    FROM ${client_1.Prisma.raw(TABLE_DESEMBOLSO)} d
    INNER JOIN (
      SELECT
        nr_convenio_norm,
        MAX(cnpj) AS cnpj,
  ...
  `;
    ...
    const offset = (page - 1) * pageSize;
    const [countRows, rows, resumoRows, syncState] = await Promise.all([
        prisma_1.prisma.$queryRaw(client_1.Prisma.sql `SELECT COUNT(*) AS total ${fromClause} ${whereClause}`),
        prisma_1.prisma.$queryRaw(client_1.Prisma.sql `
      SELECT
        d.id,
        d.id_desembolso,
...
exports.listarDesembolsosPorProponenteTransferenciasDiscricionarias = listarDesembolsosPorProponenteTransferenciasDiscricionarias;
const listarSugestoesProponentePorCnpj = async (cnpj, limit = 10, user) => {
    await ensureTables();
    // Se for usuário demo, filtrar apenas pelos CNPJs do ambiente demo
    let cnpjsDoAmbiente;
    if (isDemoUser(user)) {
    ...
    let whereClause = client_1.Prisma.sql `cnpj IS NOT NULL AND nome_proponente IS NOT NULL AND cnpj LIKE ${like} ESCAPE '\\'`;
    if (cnpjsDoAmbiente && cnpjsDoAmbiente.size > 0) {
        const cnpjList = [...cnpjsDoAmbiente].map((c) => client_1.Prisma.sql `${c}`);
        whereClause = client_1.Prisma.sql `${whereClause} AND cnpj IN (${client_1.Prisma.join(cnpjList, ",")})`;
    }
...
exports.listarSugestoesProponentePorCnpj = listarSugestoesProponentePorCnpj;
const sincronizarTransferenciasDiscricionarias = async (optionsInput = false, user) => {
    ...
    const cnpjsValidos = new Set(convenetes.map((c) => c.cnpj.replace(/\D/g, "")).filter((cnpj) => cnpj.length > 0));
    // Verifica se temos registros para todos os CNPJs atuais
    let temNovosProponentesSemDados = false;
    if (cnpjsValidos.size > 0) {
        const cnpjsComDados = await prisma_1.prisma.$queryRaw `
    ...
    const updateSyncProgress = async (faseAtual, progressoPercentual) => {
    ...
    const runWithSyncHeartbeat = async (faseAtual, progressoPercentual, task, heartbeatIntervalMs = 15000) => {
        let stopped = false;
    ...
    const conveniosValidos = new Set();
    const propostasValidas = new Set();
    try {
        await assertSyncNotCancelled();
        await updateSyncProgress("Limpando area de preparacao", 8);
        await prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_STAGE}`);
        await prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_PROPOSTA_STAGE}`);
        await prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_DESEMBOLSO_STAGE}`);
        ...
        for (let index = 0; index < sourceFiles.length; index++) {
            ...
            await assertSyncNotCancelled();
            try {
                const csvText = await (0, exports.downloadCsvTextFromZipWithRetry)(sourceFile);
        ...
        await assertSyncNotCancelled();
        try {
            const csvText = await (0, exports.downloadCsvTextFromZipWithRetry)(DESEMBOLSO_FILE, "latin1");
        ...
        await assertSyncNotCancelled();
        await prisma_1.prisma.$executeRawUnsafe(`UPDATE ${TABLE_STAGE} SET sync_run_id = '${runId.replace(/'/g, "''")}'`);
        await prisma_1.prisma.$executeRawUnsafe(`UPDATE ${TABLE_DESEMBOLSO_STAGE} SET sync_run_id = '${runId.replace(/'/g, "''")}'`);
        await prisma_1.prisma.$executeRawUnsafe(`
      UPDATE ${TABLE_STAGE}
      SET
    ...
    `);
        const validation = await validateStageBeforePublish(warnings, sourceFiles);
        if (!validation.ok) {
        ...
        await prisma_1.prisma.$transaction([
            prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_MAIN}`),
            prisma_1.prisma.$executeRawUnsafe(`
        ...
        FROM ${TABLE_STAGE}
      `),
            prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_STAGE}`),
            prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_PROPOSTA_STAGE}`),
            prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_DESEMBOLSO}`),
            prisma_1.prisma.$executeRawUnsafe(`
      ...
      `),
            prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_DESEMBOLSO_STAGE}`)
        ]);
        ...
        if (mode === "full") {
            ...
            await assertSyncNotCancelled();
            try {
                await runWithSyncHeartbeat("Promovendo instrumentos oficiais", 97, async () => (0, convenetes_service_1.reimportarInstrumentosTodosProponentesAtendidos)(user));
            }
            catch (promoError) {
            ...
            await assertSyncNotCancelled();
            try {
                await runWithSyncHeartbeat("Sincronizando obras e medicoes", 99, async () => (0, obras_service_1.sincronizarObrasTransferegov)());
            }
            catch (obrasError) {
        ...
        try {
            const notifyResult = await (0, transferencias_discricionarias_changes_notify_service_1.monitorarAlteracoesFinanceirasTransferenciasDiscricionarias)("sync");
        ...
        await writeSyncState({
            ...
            atualizado_em: new Date().toISOString(),
            status: "error",
            detalhe: error instanceof SyncCancelledError
        ...
        await updateSyncRun(runId, {
            ...
            data_carga_fonte: dataCargaFonte,
            mode,
            detail: error instanceof SyncCancelledError
...
exports.sincronizarTransferenciasDiscricionarias = sincronizarTransferenciasDiscricionarias;
const obterStatusSincronizacaoTransferenciasDiscricionarias = async () => {
    const state = await readSyncState();
...
exports.obterStatusSincronizacaoTransferenciasDiscricionarias = obterStatusSincronizacaoTransferenciasDiscricionarias;
const complementarConveniosAusentesTransferenciasDiscricionarias = async (user) => {
    await ensureTables();
    const state = await readSyncState();
    ...
    const cnpjsValidos = new Set(convenetes.map((item) => item.cnpj.replace(/\D/g, "")).filter((cnpj) => cnpj.length > 0));
    await prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_STAGE}`);
    await prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_PROPOSTA_STAGE}`);
    const totalStage = await enriquecerDadosPropostaNaStage(new Set(), cnpjsValidos, dataCargaFonte);
    await enriquecerIngressoContrapartidaNaStage(new Set());
    await prisma_1.prisma.$executeRawUnsafe(`
    INSERT INTO ${TABLE_MAIN} (
      chave_unica,
      fonte_arquivo,
  ...
  `);
    await prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_STAGE}`);
    await prisma_1.prisma.$executeRawUnsafe(`DELETE FROM ${TABLE_PROPOSTA_STAGE}`);
    const importacao = await (0, convenetes_service_1.reimportarInstrumentosTodosProponentesAtendidos)(user);
    return {
        data_carga_fonte: dataCargaFonte,
...
exports.complementarConveniosAusentesTransferenciasDiscricionarias = complementarConveniosAusentesTransferenciasDiscricionarias;
const cancelarSincronizacaoTransferenciasDiscricionarias = async () => {
    const state = await readSyncState();
...
exports.cancelarSincronizacaoTransferenciasDiscricionarias = cancelarSincronizacaoTransferenciasDiscricionarias;
